...
// ── Render Staff Panel ──────────────────────
// Quick-assign a person's home facility straight from the team list — saves
// instantly and refreshes the name pickers so the change shows everywhere.
async function staffSetFarm(fbId, farm) {
  const s = (typeof staffList !== 'undefined' && Array.isArray(staffList)) ? staffList.find(x => x._fbId === fbId) : null;
  if (s) s.farm = farm;
  renderStaff();
  if (typeof updateStaffDropdowns === 'function') updateStaffDropdowns();
  try {
    if (typeof setSyncDot === 'function') setSyncDot('saving');
    await db.collection('staff').doc(fbId).update({ farm });
    if (typeof setSyncDot === 'function') setSyncDot('live');
  } catch (e) {
    console.error('staffSetFarm:', e);
    if (typeof setSyncDot === 'function') setSyncDot('live');
  }
}

    ...
    tasks: [
      ...
      },
      {
        ...
        simple: [
          ...
          { en: 'Anyone can pick up a check already started for that house — it is shared live across all iPads, so you see which sections are done, who did each, and what is left. No fresh start.', es: 'Cualquiera puede continuar un chequeo ya empezado en esa casa — se comparte en vivo entre todas las iPads, así ves qué secciones están hechas, quién hizo cada una y qué falta. No empieza de cero.' },
        ...
        detail: [
          ...
          { en: 'Completed blocks collapse to keep the screen short and show who finished each one; tap a block header to reopen and change an answer.', es: 'Los bloques terminados se cierran para acortar la pantalla y muestran quién terminó cada uno; toca el título de un bloque para volver a abrirlo y cambiar una respuesta.' },
          { en: 'Progress is saved live as the crew works, so a check can be handed off between people or iPads without losing anything.', es: 'El avance se guarda en vivo mientras el equipo trabaja, así un chequeo se puede pasar entre personas o iPads sin perder nada.' },
          { en: 'A house not submitted by midnight is counted INCOMPLETE and cleared — each day starts fresh and nothing partial is ever auto-turned-in.', es: 'Una casa no enviada antes de medianoche cuenta como INCOMPLETA y se borra — cada día empieza de cero y nada parcial se entrega solo.' },

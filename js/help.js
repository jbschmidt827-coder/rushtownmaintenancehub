...
const HELP_CONTENT = [
  ...
  },
  {
    ...
    name: 'Feed Mill',
    color: '#eab308',
    blurb: 'Feed made · Bulk entry · Daily grid',
    tasks: [
      {
        title: '🌾 Log Feed Made',
        simple: [
          'Open the Feed Mill screen → Feed Made.',
          'Check the Date.',
          'Enter Tons Made (required).',
          'Pick the Type, the Destination (own barns or external), the Farm and House.',
          'Put your name in "By" and tap Save.',
        ],
        detail: [
          'Date and Tons Made are required; everything else is optional but Farm + House make the Daily Grid accurate.',
          'Destination "own" feeds your barns; "external" is feed sold/sent out.',
          'To enter a whole farm at once, use Bulk Feed Made — type tons for each house and save them all together.',
        ],
      },
      {
        title: '📊 Read the Daily Grid',
        simple: [
          'Feed Mill → Daily Grid tab.',
          'It shows a 14-day tons matrix per farm and house.',
          'Each saved Feed Made entry drops into the right day and house automatically.',
        ],
        detail: [
          'Entries with no farm assigned fall into an "Unassigned" row — set the farm when logging to avoid this.',
          'Use the grid to spot missed days or unusual tonnage at a glance.',
        ],
      },
    ],
  },
  {
    ...
    color: '#a78bfa',
    blurb: 'Packaging PMs · Plant schedules',
    tasks: [
      {
        title: '📦 Packaging PMs (100–126)',
        simple: [
          'Make sure your location is set to the plant.',
          'Open Maintenance → PM list — packaging PMs are numbered 100 through 126.',
          'Tap a PM, check off each step, then tap Mark Done and sign your name.',
        ],
        detail: [
          'These 18 packaging PMs are locked to the Processing Plant so they only show at the plant.',
          'Same rule as all PMs: every step must be checked before Mark Done unlocks.',
        ],
      },
      {
        title: '🕐 Plant Start Times',
        simple: [
          'Hegins: 5:30 start · 6:00 AM walk · 8:00 break · 12:00 lunch.',
          'Danville: 7:00 AM start.',
          'The app already knows your plant\'s schedule once you pick your location.',
        ],
        detail: [
          'Schedules live in the in-app Schedule editor and can be adjusted by a supervisor without a new release.',
          'Late walks past the scheduled time get auto-flagged.',
        ],
      },
    ],
  },

...
function openMorningWalk(farm, house) {
  ...
  ['mw-employee','mw-water','mw-temp','mw-notes','mw-ee-count','mw-feed-meter','mw-bin-a','mw-bin-b'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  ['mw-bin-a-status','mw-bin-b-status','mw-feed-meter-status'].forEach(id => {
...
function mwCheckBinLevel(inputId, statusId) {
  ...
  if (!el || !st) return;
  const val = el.value;
  if (val === '' || val === null) { st.textContent = ''; el.style.borderColor = '#1e3a6a'; return; }
  const n = Number(val);
  if (n < 2) {
    st.style.color = '#f87171';
    st.textContent = '⚠ Low — order feed';
    el.style.borderColor = '#ef4444';
  } else if (n < 5) {
    st.style.color = '#fbbf24';
    st.textContent = '↓ Getting low';
...
async function submitMorningWalk() {
  ...
  const binA = document.getElementById('mw-bin-a')?.value !== '' && document.getElementById('mw-bin-a')?.value !== undefined ? Number(document.getElementById('mw-bin-a').value) : null;
  const binB = document.getElementById('mw-bin-b')?.value !== '' && document.getElementById('mw-bin-b')?.value !== undefined ? Number(document.getElementById('mw-bin-b').value) : null;
  if (binA !== null && binA < 2) flags.push('Bin A low: ' + binA + ' tons');
  if (binB !== null && binB < 2) flags.push('Bin B low: ' + binB + ' tons');
  const record = {

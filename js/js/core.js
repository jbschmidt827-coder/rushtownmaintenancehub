...
async function initApp() {
  ...
  try {
    ...
    startContractorListener();
    startSignoffListener();
    startRedTagListener();
    await loadOpsData();
    startOpsListeners();
...
function go(tab) {
  ...
  const maintSections = {wo:'wo', pm:'pm', parts:'parts', downtime:'downtime', log:'log', assets:'assets', wi:'wi', '5s':'5s', calendar:'calendar', contractor:'contractor', cost:'cost', 'weekly-agenda':'weekly-agenda', redtags:'redtags'};
  ...
  const pm = {dash:'panel-dash', prod:'panel-prod', maint:'panel-maint', pkg:'panel-pkg', feed:'panel-feed', ship:'panel-ship', kpi:'panel-kpi', reports:'panel-reports', sched:'panel-sched', staff:'panel-staff', check:'panel-check', mw:'panel-mw', daily:'panel-daily', brief:'panel-brief'};
  const tm = {dash:'tab-dash', prod:'tab-prod', maint:'tab-maint', pkg:'tab-pkg', feed:'tab-feed', ship:'tab-ship', kpi:'tab-kpi', reports:'tab-reports', sched:'tab-sched', staff:'tab-staff', check:'tab-check', mw:'tab-mw', daily:'tab-daily', brief:'tab-brief'};
  if (!pm[tab]) return;
  ...
  if (tab === 'brief')  renderDirectorBrief();
}

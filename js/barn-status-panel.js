...
(function () {
  ...
  function render() {
    ...
    let started = 0, submitted = 0, failingNow = 0;
    let aggDone = 0;
    const allBarns = [];
    ...
    Object.entries(FARMS).forEach(([farm, count]) => {
      ...
      for (let b = 1; b <= count; b++) {
        ...
        if (!rec) {
          // Not started
          html += `<div title="Not started" style="background:#0a1a0a;border:1.5px solid #1a2a1a;border-radius:9px;padding:7px 5px;text-align:center;">
        ...
        // Color choice: failing > submitted > in-progress > stale
        ...
        html += `<div title="${farm} H${b} — ${fmtMin(dn)} of ${fmtMin(planned)} done · ${rec.worker||'?'} · ${ago}" style="background:${bg};border:1.5px solid ${bdr};border-radius:9px;padding:7px 5px;text-align:center;">
    ...
    const aggPct = (planned * totalBarns) > 0
      ...
      : 0;
    html += `
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:6px;padding-top:8px;border-top:1px solid #1a3a1a;">
